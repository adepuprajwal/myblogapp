...
function ArticleByID() {
  ...
  async function addComment(commentObj){
    ...
    // http put
    let res = await axios.put(`http://localhost:3000/user-api/comment/${currentArticle.articleId}`,
      commentObj,
      {
        headers: {
          Authorization: `Bearer ${token}`
    ...
    <div className='container'>
      {
        editArticleStatus === false ? <>
          ...
          <div className="d-flex justify-contnet-between">
            <div className="mb-5 w-100 px-4 py-2 rounded-2 d-flex justify-content-between align-items-center" style={{backgroundColor:'#A1E3F9'}} >
              <div>
            ...
            {/* edit & delete */}
            {
              currentUser.role === 'author' && (
                <div className="d-flex me-3">
                  {/* edit button */}
                  <button className="mx-2 btn btn-light" style={{height:'50px'}} onClick={enableEdit}>
                    <FaEdit style={{color:'#578FCA'}} />
                  </button>
                  {/* if article is active,display delete icon, otherwise display restore icon */}
                  {
                    state.isArticleActive === true ? (
                      <button className="mx-2 btn btn-light" style={{height:'50px'}} onClick={deleteArticle}>
                        <MdDelete className='text-danger fs-4' />
                      </button>
                    ) : (
                      <button className="mx-2 btn btn-light" onClick={restoreArticle}>
